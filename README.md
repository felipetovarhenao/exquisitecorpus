...
* **transp. Mode**: sets the mode of transposition. 
  ...
  * **resample**: transposition affects both the pitch and original duration of the sample. High transpositions will result in shorter durations, while low transpositions will result in longer durations.
...
#### Recommendations:
*	When a corpus is loaded, **EC-sampler** automatically computes a dictionary specifying which samples are closer in pitch to each MIDI pitch value (from 22 to 108) — this is done in order to know what samples would require the least amount of transposition to get any pitch. The percentage of the closest samples to be used for each pitch is determined by the BMI parameter. In other words, it determines what percentage of the corpus will be used for every single MIDI pitch value, thus making it possible to minimize extreme sample transpositions. For reference, a BMI of 100% will use all the selected samples for every pitch value, resulting in possibly extreme pitch transpositions. A BMI of 0% will only use the sample that requires the smallest transposition amount for each pitch, resulting in a more naturally sounding corpus.  
*	Similarly, for a more naturally sounding corpus, make sure that the samples in your corpus are as evenly distributed as possible across every register. For example, if the original MIDI pitch range of all the samples is, let’s say, between a 60 and 64, this will result is extreme transpositions when trying to get pitches far outside of that range.
